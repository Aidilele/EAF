...
from module.helpers import Losses, cosine_beta_schedule, extract, history_cover
...
class GaussianInvDynDiffusion(nn.Module):
    ...
    def p_mean_variance(
        ...
        if self.returns_condition:
            ...
            epsilon_cond = self.model(x, obs, t, condition, use_dropout=False)
            epsilon_uncond = self.model(x, obs, t, condition, force_dropout=True)
    ...
    def p_sample_loop(
        ...
        for i in reversed(range(self.n_timesteps)):
            ...
            x, model_mean, model_variance = self.p_sample(x, history, timesteps, condition)
            x = history_cover(x, history, 0, self.history_lenght)
    ...
    def conditional_sample(
        ...
        return self.p_sample_loop(shape, history, condition, *args, **kwargs)
    ...
    def p_losses(
        ...
        x_recon = self.model(x_noisy, obs=history, t=t, condition=condition)
